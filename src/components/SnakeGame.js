...
function SnakeGame(props) {
  ...
  $("body").on("keydown", function (e) {
    if (e.keyCode == 37 && gameState == JUGANDO && lastDirection != RIGHT) {
      // left
      direction = LEFT;
      ...
      // up
      direction = UP;
    } else if (e.keyCode == 40 && gameState == JUGANDO && lastDirection != UP) {
      // down
      direction = DOWN;
    }
  });
  ...
  var ini = (
    <>
      ...
      <div className="d-flex justify-content-center">
        <Row>
          <Col>
            <h4 className="text-center" id="eatenFruits">
              Fruits: {eatenFruits}
            </h4>
          </Col>
          <Col>
      ...
      <h6 className="text-center">
        Start with the right arrow on the keyboard!
      </h6>
          ...
          {matrix.map(function (x, i) {
            return (
              <tr key={i}>
                {x.map(function (y, j) {
                  return (
                    <td key={j} className="snakeBox" id={i + "-" + j}>
                      {isIniBox(i, j)}
                    </td>
                  );
                })}
  ...
  function transformateHeadToBody(orientation) {
    ...
    if (eatenFruits > 1) {
      if (snake[snake.length - 2].orientation == orientation) {
        idBox =
          "#" + snake[snake.length - 2].i + "-" + snake[snake.length - 2].j;
        $(idBox).empty();
        $(idBox).prepend(
          $("<img>", {
        ...
        (snake[snake.length - 2].orientation == RIGHT && orientation == DOWN) ||
        ...
        (snake[snake.length - 2].orientation == DOWN && orientation == RIGHT) ||
  ...
  function isSnakeBody(nextMove_i, nextMove_j) {
    return snake.find(
      (item, i) => i != 0 && item.i == nextMove_i && item.j == nextMove_j
    );
  }
